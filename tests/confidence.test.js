...
import { assessmentConfidence, filterFramesByVisibility, frameVisibility, segmentConfidence } from '../js/analysis/confidence.js';
...
test('assessment confidence includes sample support', () => {
  const frames = Array.from({ length: 10 }, () => frame(1, 1));
  assert.equal(assessmentConfidence(frames, [0, 1]), 50);
});
